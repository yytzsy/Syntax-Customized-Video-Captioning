...
from models import EncoderRNN, DecoderRNN, DecoderSentenceRNN, DecoderSyntaxRNN, S2VTAttModel, S2VTModel
...
def test(model, crit, dataset, vocab, opt, epoch):
    ...
    for data in loader:
        ...
        template_labels_character = data['template_labels_character'].cuda()  #batchsize, template_sen_number, word number in sentence, max_character_len
        template_masks = data['template_masks'].cuda()  #(b,20,28)
        ...
        template_labels = np.squeeze(template_labels.cpu().numpy())
        template_labels_character = np.squeeze(template_labels_character.cpu().numpy())
        template_masks = np.squeeze(template_masks.cpu().numpy())
        template_labels = torch.from_numpy(template_labels).cuda()
        template_labels_character = torch.from_numpy(template_labels_character).cuda()
        template_masks = torch.from_numpy(template_masks).cuda()
        ...
        with torch.no_grad():
            seq_probs, seq_preds = model(fc_feats, None, None, template_labels_character, template_labels_character, None, None, None, mode='inference', opt=opt)
    ...
    if tmp_template_sentence_file.closed == False:
        tmp_template_sentence_file.close()
    if tmp_predict_sentence_file.closed == False:
        tmp_predict_sentence_file.close()
    ...
    if os.path.exists('./template_parse.pkl'):
        ref_parses = pkl.load(open('./template_parse.pkl','r'))
    else:
        ref_parses = spe.run(template_path_list)
        pkl.dump(ref_parses, open('./template_parse.pkl','w'))
...
def main(opt):
    ...
    if opt["model"] == 'S2VTModel':
        ...
        model = S2VTAttModel(encoder, decoder, decoder_sentence, decoder_syntax, opt["dim_hidden"]).cuda()
